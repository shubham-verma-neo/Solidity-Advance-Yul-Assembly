...
/*
    ...
    1. calldata load
    2. imitating function selector
    3. switch statemet 
    4. yul functions with arrguments
    5. function that returns
    6. exit from function without returning
    7. validating calldata
    */
contract CalldataDemo {
    fallback(bytes calldata data) external returns (bytes memory returnData){
        assembly {
            let cd := calldataload(0) // always load 32 bytes
